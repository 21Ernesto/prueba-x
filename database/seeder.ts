/**
 * Seeder — Venzo POS
 * Ejecutar con:  npx ts-node database/seeder.ts
 *
 * Requisitos: que la base de datos exista y el servidor MySQL esté activo.
 */
...
const ds = new DataSource({
    ...
    synchronize: true, // Habilitado para crear las tablas automáticamente al conectar
    entities: [path.join(__dirname, '../src/**/*.entity{.ts,.js}')],
});

async function run(q: DataSource, sql: string, params: any[] = []) {
...
async function seed() {
    ...
    for (const [nombre, clave, grupo] of permisos) {
        await run(ds, `INSERT IGNORE INTO permiso (nombre, clave, grupo) VALUES (?, ?, ?)`,
            [nombre, clave, grupo]);
    }
